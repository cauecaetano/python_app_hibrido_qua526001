...
def main(page: ft.Page):
    ...
    video_info = ft.Container(
        content=ft.Column([]),
        visible=False,
        padding=ft.Colors.BLUE_GREY_50,
        border_radius=10,
        width=400
    )
    ...
    status_text = ft.Text(
        "",
        colo=ft.Colors.BLACK,
        siza=14,
        text_align=ft.TextAlign.CENTER
    )

    # mostra as informações de vídeos na interface 
        ...
        try:
            ...
            video_info.content.controls.extend(
                [
                    ...
                    ft.Text(f"Visualizações: {yt.yt.views:,}", size=12),
                ]
            ) 
        ...
        if not url.value.strip():
            ...
            def donwload_thread():
                try:
                    ...
                    # inicia download
                    status_text.value = f"Baixando vídeos: {yt.title}..."
                    page.update()

                    # pega a maior resolução possível
                    stream = yt.streams.get_highest_resolution()

                    # TODO: fazer if else do streamJ
                except Exception as e:
